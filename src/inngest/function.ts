...
export const meetingsProcessing=inngest.createFunction(
    ...
    async({event,step})=>{
        ...
        const transcriptWithSpeakers=await step.run("add-speakers",async()=>{
            ...
            const userSpeakers=await db.select().from(user)
            .where(inArray(user.id,speakerIds)).then((users)=>users.map(user=>({
                ...user
            })))
            const agentSpeakers=await db.select().from(agents)
            .where(inArray(agents.id,speakerIds)).then((agents)=>agents.map(agent=>({
                ...agent
            })))
